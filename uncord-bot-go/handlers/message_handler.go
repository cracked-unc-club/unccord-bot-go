...
func OnReactionAdd(event *events.GuildMessageReactionAdd) {
	...
	if event.Emoji.Name != "⭐" {
		...
		if starCount >= config.Starboard.StarThreshold {
...
func PostToStarboard(event *events.GuildMessageReactionAdd, message *discord.Message, starCount int) {
	...
	_, err := event.Client().Rest().CreateMessage(config.Starboard.StarboardChannelID, discord.NewMessageCreateBuilder().AddEmbeds(embed).Build())
	if err != nil {
